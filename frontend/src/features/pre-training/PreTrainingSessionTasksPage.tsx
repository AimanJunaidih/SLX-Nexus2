...
import { getSessionCompanyPreTasks, syncSessionCompanyPreTasks } from '@/data-access/pre-training-tasks';
import type { PreTrainingTask } from '@/data-access/pre-training-tasks';
import type { TrainingSession } from '@/entities/training-session';

export default function PreTrainingSessionTasksPage() {
  const navigate = useNavigate();
  const { sessionId, companyId } = useParams<{ sessionId: string; companyId: string }>();
  const [session, setSession] = useState<TrainingSession | null>(null);
  const [companyName, setCompanyName] = useState('');
  const [items, setItems] = useState<PreTrainingTask[]>([]);
  ...
  useEffect(() => {
    ...
    Promise.all([getTrainingSessions(), getSessionCompanyPreTasks(sessionId, companyId)]).then(([sessions, tasks]) => {
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      const synced = await syncSessionCompanyPreTasks(sessionId, companyId, items);
